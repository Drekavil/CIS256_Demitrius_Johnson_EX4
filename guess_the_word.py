...
def guessingGame():
    ...
    #win/lose message
    if "_" not in guess:
        print(f"YOU WIN!! The word was {theWord.upper()}.")
    else:
        print(f"OH NO! You're out of attempts, the word was {theWord.upper()}.")


#starts the game on script run with option to play again after
if __name__ == "__main__":
    while True:
        guessingGame()
        tryagain = input("Would you like to playa gain? Enter Y to play again: ").lower()
        if tryagain not in "y":
